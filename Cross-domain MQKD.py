...
def main():
    ...
    # ------------------ Classical integrity verification using KR1/KR2 hashes ------------------
    # SC1 and SC2 announce H_SC1 = H(KR1), H_SC2 = H(KR2);
    # Alice/Bob compute H_AR1/H_AR2 and H_BR1/H_BR2 locally and verify equivalence.
    H_SC1 = hashlib.sha256(KR1.encode()).hexdigest()
    H_SC2 = hashlib.sha256(KR2.encode()).hexdigest()
    H_AR1 = hashlib.sha256(KR1_A.encode()).hexdigest()
    H_AR2 = hashlib.sha256(KR2_A.encode()).hexdigest()
    H_BR1 = hashlib.sha256(KR1_B.encode()).hexdigest()
    H_BR2 = hashlib.sha256(KR2_B.encode()).hexdigest()

    alice_ok_1 = (H_AR1 == H_SC1)
    alice_ok_2 = (H_AR2 == H_SC2)
    bob_ok_1 = (H_BR1 == H_SC1)
    bob_ok_2 = (H_BR2 == H_SC2)

    print("\n================ Sequence Integrity Verification via KR1/KR2 Hashes ================")
    print(f"SC1 announces H_SC1=H(KR1): {H_SC1}")
    print(f"SC2 announces H_SC2=H(KR2): {H_SC2}")
    print(f"Alice computes H_AR1=H(KR1_A): {H_AR1}  -> match with SC1: {alice_ok_1}")
    print(f"Alice computes H_AR2=H(KR2_A): {H_AR2}  -> match with SC2: {alice_ok_2}")
    print(f"Bob   computes H_BR1=H(KR1_B): {H_BR1}  -> match with SC1: {bob_ok_1}")
    print(f"Bob   computes H_BR2=H(KR2_B): {H_BR2}  -> match with SC2: {bob_ok_2}")

    integrity_ok = alice_ok_1 and alice_ok_2 and bob_ok_1 and bob_ok_2
    print(f"\nSequence integrity verification result (KR1/KR2): {integrity_ok}")
    if not integrity_ok:
        print("Warning: Sequence integrity verification failed, recommend terminating protocol!")
        return
    ...
    for i in range(N):
        ...
        MS_A2.append(ms_a2)
        MS_B2.append(ms_b2)
    # Directly concatenate keys according to protocol: KA=MS_A1||MS_A2; KB=MS_B1||MS_B2
    KA = ''.join(MS_A1) + ''.join(MS_A2)
    KB = ''.join(MS_B1) + ''.join(MS_B2)
    print("\nMeasurement strings:")
    print(f"MS_A1={MS_A1}")
    print(f"MS_A2={MS_A2}")
    print(f"MS_B1={MS_B1}")
    print(f"MS_B2={MS_B2}")
    print("\n================ Final Results ================")
    print(f"KA length: {len(KA)}, KB length: {len(KB)}, Expected length: {4*N}")
    print(f"KA: {KA}")
    print(f"KB: {KB}")
